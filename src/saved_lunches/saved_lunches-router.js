...
SavedLunchRouter.route("/")
  ...
  .post(jsonParser, (req, res, next) => {
    ...
    SavedLunchService.insertList(req.app.get("db"), newList)
      .then((list) => {
...
SavedLunchRouter.route("/:id")
  .all(requireAuth)
  .all((req, res, next) => {
    SavedLunchService.getById(req.app.get("db"), req.params.id)
      .then((list) => {
        if (!list) {
  ...
  .delete((req, res, next) => {
    SavedLunchService.deleteList(req.app.get("db"), req.params.id)
      .then(() => {
        res.status(204).end();
      })
      .catch(next);
  ...
  .patch(jsonParser, (req, res, next) => {
    ...
    SavedLunchService.updateList(req.app.get("db"), req.params.id, listToUpdate)
      .then((result) => {
        res.status(204).end();
      })
      .catch(next);
...
SavedLunchRouter.route("/users/:user_id")
  .all(requireAuth)
  .all((req, res, next) => {
    SavedLunchService.getByUserId(req.app.get("db"), req.params.user_id)
      .then((list) => {
        if (!list[0]) {
          return res.status(404).json({
            error: { message: `List doesn't exist` },
